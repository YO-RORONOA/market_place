...
<h1 class="text-2xl font-bold text-accent-navy mb-6">Edit Product</h1>

<form action="/vendor/products/update" method="post" enctype="multipart/form-data" id="productForm">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                ...
                <div class="mb-4">
                    ...
                    <textarea 
                        ...
                        required
                        class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-accent-teal focus:border-accent-teal"
                    ><?= htmlspecialchars($model->description) ?></textarea>
                ...
                <div class="mb-4">
                    <?php if (!empty($model->image_path)): ?>
                        <div class="mb-3">
                            ...
                            <img src="<?= htmlspecialchars($model->image_path) ?>" alt="Current product image" class="w-full h-48 object-cover rounded-md">
                        </div>
                    <?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    generateDescBtn.addEventListener('click', function() {
        ...
        // Make request to generate description using XHR
        const xhr = new XMLHttpRequest();
        xhr.open('POST', '/vendor/products/generate-description', true);
        ...
        xhr.onload = function() {
            if (xhr.status >= 200 && xhr.status < 400) {
                try {
                    ...
                    aiSpinner.classList.add('hidden');
                    aiError.textContent = 'Invalid response from server';
                    aiError.classList.remove('hidden');
                    console.error('Error parsing JSON:', e);
                }
            } else {
                aiSpinner.classList.add('hidden');
                aiError.textContent = 'Failed to generate description. Server returned an error.';
                aiError.classList.remove('hidden');
        ...
        xhr.onerror = function() {
            aiSpinner.classList.add('hidden');
            aiError.textContent = 'Failed to generate description. Network error occurred.';
            aiError.classList.remove('hidden');
            console.error('Network error occurred');
        };
        
        // Format data for x-www-form-urlencoded
        const formData = 'name=' + encodeURIComponent(productName) + 
